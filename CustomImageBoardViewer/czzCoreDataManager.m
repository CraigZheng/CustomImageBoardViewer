...
@implementation czzCoreDataManager
...
-(void)insertThreadIntoContext:(czzThread *)thread {
    ...
    NSError *error;
    if ([newThread copyPropertiesFromThread:thread] && [self.managedObjectContext save:&error]) {
        DLog(@"Thread properly inserted");
    } else {
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"ThreadModel" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [[NSURL fileURLWithPath:[czzAppDelegate libraryFolder]] URLByAppendingPathComponent:@"ThreadCache.sqlite"];
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:[storeURL absoluteString]]) {
        NSLog(@"file not found: %@", [storeURL absoluteString]);
    } else {
        DLog(@"database file: %@", [storeURL absoluteString]);
    }
